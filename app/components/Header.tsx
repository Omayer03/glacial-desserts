...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 z-20 w-full border-b border-white/10 bg-espresso/30 backdrop-blur-md">
          ...
          <ul className="flex items-center gap-8 font-sans text-xs font-medium uppercase tracking-[0.15em] text-white/80">
            {navLinks.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="relative pb-1 transition-colors after:absolute after:bottom-0 after:left-0 after:h-px after:w-0 after:bg-gold after:transition-all after:duration-300 after:ease-out hover:text-gold hover:after:w-full"
                >
                  {link.label}
                </a>
              </li>
      ...
      {menuOpen && (
        <nav className="border-t border-white/10 bg-espresso/95 backdrop-blur-md md:hidden">
            ...
            {navLinks.map((link) => (
              <li key={link.label}>
                <a
                  ...
                  className="relative block w-fit py-3 transition-colors after:absolute after:bottom-2 after:left-0 after:h-px after:w-0 after:bg-gold after:transition-all after:duration-300 after:ease-out hover:text-gold hover:after:w-full"
                >
                  {link.label}
                </a>
              </li>
